...
def my_status(task):
    ...
    if task['last_failed'] is not None and task['last_success'] is not None and task['last_failed'] > task[
        'last_success']:
        return '失败'
...
class MyHandler(BaseHandler):
    ...
    def get(self):
        ...
        tasks = []
        for task in self.db.task.list(user['id'], fields=(
        'id', 'tplid', 'note', 'disabled', 'last_success', 'success_count', 'failed_count', 'last_failed', 'next',
